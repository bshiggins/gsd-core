 ...
 *
 * RE-PORT NOTE: bracket is ADDITIVE (opt-in via `--convention bracket`), not the
 * default. `milestone-prefixed` stays the migrator default and is untouched — current
 * upstream kept it as a shipped/tested convention. So every invocation here passes
 * `--convention bracket` explicitly (the banked oracle assumed bracket-default).
 *
 * Under `--convention bracket` the migrator emits BRACKET as the terminal form:
 *   - dir:      {CODE}.{MM}-{SS}-slug          (GSD.02-01-foo)
 *   - heading:  ### [{CODE}.{MM}] {SS}: Name
 *   - checklist:- [ ] **[{CODE}.{MM}] {SS}: Name**
 *   - config:   phase_id_convention: 'bracket'
 * For a bracket target, M-NN is a convertible SOURCE: `Phase 2-01` lifts to
...
function plan(tmpDir) {
  const r = runGsdTools(['roadmap', 'upgrade', '--convention', 'bracket'], tmpDir);
...
`);
    ...
    const r = runGsdTools(['roadmap', 'upgrade', '--apply', '--convention', 'bracket'], tmpDir);
