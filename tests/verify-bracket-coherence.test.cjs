...
// ===========================================================================
// B6 — milestone-complete check is bracket-tolerant
// (B5 W021->W022 renumber DROPPED in re-port: current upstream pins W021 here
//  via tests/bug-557; the renumber is out of scope for bracket. The check keeps
//  W021. Only B6 — making the check SEE bracket headings — is carried forward.)
// ===========================================================================

describe('PR2-C: milestone-complete check is bracket-tolerant (#612 B6)', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempProject(); });
  afterEach(() => cleanup(tmpDir));

  test('bracket phase heading counts as a phase (B6): bracket-tolerant pattern sees it', () => {
    writeProject(tmpDir);
    writeConfig(tmpDir, { phase_id_convention: 'bracket' });
    writeState(tmpDir, '---\nstatus: milestone complete\n---\n# Session State\n');
    // A bracket phase heading with no on-disk dir. Without the
    // PHASE_HEADING_PREFIX_SRC edit the check's literal 'Phase' regex misses it
    // (goes blind under bracket); with it, the heading is recognized as unstarted.
    writeRoadmap(tmpDir,
`# Roadmap
...
`);
    const out = health(tmpDir);
    const hit = w021s(out).filter((w) => /unstarted|milestone complete/i.test(w.message));
