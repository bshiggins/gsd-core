...
- Creating the phase directory (`.planning/phases/{PROJECT}.{MM}-{N.M}-{slug}/`)
- Inserting the phase entry into ROADMAP.md after the target phase as a bracket heading (`### [{PROJECT}.{MM}] {N.M}: Name`) with (INSERTED) marker
...
```
[{PROJECT}.{MM}] {decimal_phase} inserted after [{PROJECT}.{MM}] {after_phase}:
- Description: {description}
- Directory: .planning/phases/{PROJECT}.{MM}-{decimal-phase}-{slug}/
...
**[{PROJECT}.{MM}] {decimal_phase}: {description}** -- urgent insertion
...
**Also available:**
- Review insertion impact: Check if [{PROJECT}.{MM}] {next_integer} dependencies still make sense
- Review roadmap
