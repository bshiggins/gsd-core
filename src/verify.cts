    ...
    if (fs.existsSync(statePath) && fs.existsSync(roadmapPath)) {
      ...
      if (isMarkedComplete) {
        ...
        const phasePattern = new RegExp(
          `#{2,4}\\s*${core.PHASE_HEADING_PREFIX_SRC}(\\d+[A-Z]?(?:\\.\\d+)*)\\s*:\\s*([^\\n]+)`,
          'gi',
        );
        ...
        if (unstarted.length > 0) {
          // #612 (B5): renumbered off the W021 collision. This milestone-complete
          // check is unrelated to the milestone-coherence W021 family above; it
          // gets its own code W022 (was a latent two-checks-one-code collision).
          addIssue(
            'warning',
            'W022',
